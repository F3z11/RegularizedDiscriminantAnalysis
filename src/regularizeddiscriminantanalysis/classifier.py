...
class RegularizedDiscriminantAnalysis(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, lambda_: float = 0.0, gamma: float = 0.0, 
                 priors: Optional[np.ndarray] = None, reg_param: float = 1e-6):
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> 'RegularizedDiscriminantAnalysis':
        ...
        if self.priors is None:
            self.priors_ = self.class_counts_ / n_samples
        else:
        ...
        self.pooled_cov_ = np.average(self.covariances_, axis=0, weights=self.class_counts_)

        # 3. Apply Regularization (Lambda & Gamma) - Pre-compute inverses
        self._apply_regularization(n_samples, n_features)
        
        return self

    def _apply_regularization(self, n_samples, n_features):
        """
        ...
        for k in range(self.n_classes_):
            ...
            self.regularized_covariances_.append(Sigma_final_k)
            ...
            self.precisions_.append(pinv(Sigma_final_k))
